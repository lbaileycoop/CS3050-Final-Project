...
class AI:
    ...
    def update_cross_checks_letters(self):
        ...
        for tile in self.board.get_current_turn_tiles():
            ...
            across = bool(self.board.get_current_turn_tiles()[0].coords[0] -
            ...
            if across:
                ...
                if coords[0]-1 > -1:
                    CROSS_CHECKS_ACROSS[
                        coords[0]-1][coords[1]] = {
                            letter for letter in ALPHABET if
                            valid_word(
                                letter+tiles_to_str(self.board.find_string(coords, 1, 0))[0])}

                if coords[0]+1 < ROWS:
                    CROSS_CHECKS_ACROSS[
                        coords[0]+1][coords[1]] = {
                            letter for letter in ALPHABET if
                            valid_word(tiles_to_str(
                                self.board.find_string(coords, -1, 0))[::-1]+letter)}
                ...
                if coords[1]+1 < COLS:
                    CROSS_CHECKS_DOWN[
                        coords[0]][coords[1]+1] = {
                            ...
                            if valid_word(
                                tiles_to_str(self.board.find_string(coords, 0, -1))[::-1]+letter)}
