"""Module to run a Scrabble game using PyArcade"""
import arcade
...
class ScrabbleUI(arcade.View):
    """Class to display and run the backend of a scrabble game"""
    ...
    def __init__(self):
        ...
        for i in range(ROWS * COLS):
            ...
            tile.text_sprite.center_x = tile.sprite.center_x
            tile.text_sprite.center_y = tile.sprite.center_y

            self.board.append(tile.sprite)
            self.board.append(tile.text_sprite)
    ...
    def from_coords(self, coords: tuple[int, int]) -> int:
...
class Tile():
    """Class that represents a Scrabble tile"""

    def __init__(self, letter: str, value: int, is_empty: bool = False):
        self.sprite = arcade.Sprite(BASE_TILE)
        self.text_sprite = arcade.create_text_sprite(letter, arcade.color.BLACK, font_size=24)
...
def main():
    """Main function to initialize and run ScrabbleUI game"""
